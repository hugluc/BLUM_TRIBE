...
while True:
    ...
    print(Fore.YELLOW + Style.BRIGHT + f"2. TapTap Discussion Airdrop")
    ...
    if tribe_selection == "1":
        ...
        tribe_id = "fc8008d7-dedd-4a26-b88a-6f0dc2ac5feb"
        break
    elif tribe_selection == "3":
        print(Fore.YELLOW + Style.BRIGHT + "HAH ??? NGAWURRR !!!, MODIF THE CODE BY YOURSELF IF YOU WANT TO CUSTOM JOIN THE TRIBE !!")
        print(Fore.YELLOW + Style.BRIGHT + "Using default tribe: TapTap Discussion Airdrop")
        tribe_id = "fc8008d7-dedd-4a26-b88a-6f0dc2ac5feb"
        break
    else:
...
def join_tribe(token, tribe_id):
    ...
    response = requests.post(url, headers=headers)
    if response.status_code == 200:
        print(f"{Fore.GREEN+Style.BRIGHT}[ Tribe ] Joined PENCAIRAN BANSOS (Public)")
    else:
        print(f"{Fore.RED+Style.BRIGHT}[ Tribe ] Failed to join PENCAIRAN BANSOS (Public)")
        ...
        for index, query_id in enumerate(query_ids, start=1):
            ...
            while balance_info['playPasses'] > 0:
                ...
                if game_response is None:
                    ...
                    break
                time.sleep(10)
                claim_response = claim_game(token, game_response['gameId'], random.randint(234, 271))
                while True:
